...
def test_llm_calls_rls_has_no_anon_or_authenticated_policy(db_conn):
    ...
    with db_conn.cursor() as cur:
        ...
        quals = [r[0] for r in cur.fetchall()]
    assert quals, "llm_calls must have RLS policies defined"
    assert all(q == "(auth.role() = 'service_role'::text)" for q in quals)
...
def test_user_deletion_nulls_fk_instead_of_dropping_the_row(db_conn, make_user):
    ...
    assert row[2] == pytest.approx(0.001)
