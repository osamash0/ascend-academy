...
UPDATES = {
    ...
    "AI-01": {"test": "BLOCKED→PASS", "errors": "Backend un-importable: tutor.py:124 IndentationError (orphaned dead code from removed refusal helper) broke main + all tutor-importing routers. Import-checks pass after fix.", "severity": "Critical", "fix": "Removed orphaned `return False`/`return True` at tutor.py:123-125.", "retest": "PASS (2026-07-18): tutor.py:95-135 reviewed, no orphaned returns remain; test_tutor_grounding.py 7/7 pass; full backend suite 638+ pass."},
    "AI-04": {"test": "PASS", "errors": "_extract_citations compiles+imports clean after tutor.py fix.", "severity": "—"},
    "AI-21": {"test": "PASS (behavior change confirmed)", "errors": "Out-of-scope refusal short-circuit was intentionally removed; the dead `return False/True` remnant caused the syntax error. Stale test test_tutor_grounding::test_out_of_scope_refusal_short_circuits_llm still asserts old behavior.", "severity": "Low", "fix": "Removed dead remnant (same as AI-01). Stale test to be reconciled.", "retest": "PASS (2026-07-18): test_tutor_grounding.py no longer contains a stale refusal-short-circuit test (reconciled in an earlier commit); current 7 tests all pass and match live behavior."},
    ...
    "PDF-05": {"test": "Partial (confirmed)", "errors": "use_blueprint only threaded into the legacy parser; ignored by v4/v5.", "severity": "Low", "fix": "DEFERRED — dev/Pipeline-Lab-only option; harmless no-op. Left as-is (documenting rather than removing a tester control).", "retest": "N/A — no code change made (deferred pending product decision); nothing to retest."},
    "PDF-27": {"test": "Partial (confirmed)", "errors": "Pipeline Test Lab reads slide._meta + question.answer (v4 shape); v5 emits flat slides + correctAnswer index → route/layout/token stats empty, deck correct-option highlight wrong.", "severity": "Low", "fix": "DEFERRED — professor-only internal dev inspector, not a student/prof production surface. Tracked for a v5-aware rework.", "retest": "N/A — no code change made (deferred, v5-aware rework not yet scheduled); nothing to retest."},
