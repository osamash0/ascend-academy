...
export async function fetchStudentMastery(userId: string): Promise<StudentMastery> {
  ...
  const res = await fetch(`${API_BASE}/api/v1/concepts/student/${userId}`, { headers, credentials: 'include' });
...
export async function fetchLectureConcepts(lectureId: string): Promise<LectureConcept[]> {
  const headers = await authHeader();
  const res = await fetch(`${API_BASE}/api/v1/concepts/lecture/${lectureId}`, { headers, credentials: 'include' });
...
export async function fetchRelatedLectures(
  ...
  const headers = await authHeader();
  const url = new URL(`${API_BASE}/api/v1/concepts/${conceptId}/related-lectures`);
