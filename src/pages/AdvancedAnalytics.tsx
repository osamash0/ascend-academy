        ...
        {isOpen && (
          ...
          >
            <div className="flex items-center gap-2 mb-3 text-primary">
              ...
              <span className="text-[10px] font-black uppercase tracking-widest">AI Interpretation</span>
            </div>
            {isLoading ? (
              ...
              <p className="text-sm font-medium text-foreground/90 leading-relaxed animate-in fade-in slide-in-from-top-2 duration-500">
                {interpretation || "Select a metric to see what it means."}
              </p>
            )}
          </motion.div>
        )}
    ...
    <div className="p-6 lg:p-10 space-y-10 relative z-10">
      <div className="max-w-4xl">
        ...
        <p className="text-body-lg text-muted-foreground mt-3 max-w-2xl">
          Detailed performance data for your lectures. Select one to begin.
        </p>
      </div>
      ...
      {lectures?.length === 0 ? (
        <div className="glass-card rounded-3xl border-white/5 p-16 text-center">
          ...
          <p className="text-muted-foreground font-bold uppercase tracking-widest text-xs">No lectures yet</p>
          <p className="text-muted-foreground/60 text-sm mt-2">Upload a lecture to start tracking engagement.</p>
        </div>
      ) : (
...
export default function AdvancedAnalytics() {
  ...
  useEffect(() => {
    if (!selectedLectureId) {
      setResolvedLectureId(null);
      return;
    }
    const isUuid = /^[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}$/i.test(selectedLectureId);
    if (isUuid) {
      setResolvedLectureId(selectedLectureId);
    } else if (lectures.length > 0) {
      const matched = lectures.find(l => toSlug(l.title) === selectedLectureId);
      if (matched) {
        setResolvedLectureId(matched.id);
      } else {
        setResolvedLectureId(null);
      }
    }
  ...
  const handleMetricClick = async (id: string, title: string, value: unknown) => {
    ...
    try {
      ...
      console.error(err);
      const errorMsg = "Couldn't load feedback for this metric.";
  ...
  useEffect(() => {
    if (dashboard.isError) {
      toast.error("Couldn't load lecture analytics.");
    }
  }, [dashboard.isError]);

  // Sync title — use resolvedLectureId (UUID) to find the lecture
  useEffect(() => {
    if (resolvedLectureId) {
      if (lectures?.length > 0) {
        const lec = lectures.find(l => l.id === resolvedLectureId);
        if (lec) setSelectedTitle(lec.title);
      } else if (!lecturesLoading) {
         // Best effort if not in owned list
         setSelectedTitle('Untitled lecture');
      }
    }
      ...
      <div className="p-6 lg:p-10 space-y-10 relative z-10">
        ...
        <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-6 glass-panel p-6 rounded-[2rem] border-border/50 shadow-xl">
          <div className="flex items-center gap-5">
            <Button 
              variant="ghost" 
              size="icon" 
              onClick={() => navigate(selectedLectureId ? `/professor/analytics/${selectedLectureId}` : '/professor/analytics')}
            ...
            <div>
              ...
              <p className="text-xs font-bold text-muted-foreground uppercase tracking-[0.2em] mt-1 opacity-70">Lecture Analytics Dashboard</p>
            </div>
          </div>
        ...
        {dashboard.isLoading ? (
          ...
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="space-y-10">
                ...
                >
                  ...
                  <div className="flex flex-col lg:flex-row gap-10 items-start relative z-10">
                    ...
                    <div className="flex-1 w-full">
                      <h2 className="text-2xl font-black flex items-center gap-4 text-white mb-6">AI Summary</h2>
                      {aiLoading ? (
                        ...
                        <div className="space-y-8">
                          <p className="text-xl leading-relaxed text-white/90 font-medium max-w-5xl border-l-4 border-primary/30 pl-6 italic">
                            "{aiInsights?.summary || 'Analyzing your data...'}"
                          </p>
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-6">
              {[
                ...
                { title: "Total Events", value: dashboardData.overview.totalEvents, icon: Activity, variant: "default" as const },
                    ...
                    {selectedMetric === stat.title && (
                      ...
                      >
                        <div className="flex items-center gap-2 mb-2">
                          ...
                          <span className="text-[10px] font-black uppercase tracking-widest text-primary">AI Feedback</span>
                        </div>
                        {metricLoading ? (
                ...
                <Table className="w-full">
                  <TableHeader className="bg-white/5">
                    <TableRow className="border-white/5 hover:bg-transparent">
                      ...
                      <TableHead className="px-10 py-6 text-[10px] text-muted-foreground uppercase font-black tracking-[0.25em]">Risk Level</TableHead>
          ...
          <div className="text-center py-20">
            ...
            <p className="text-muted-foreground font-bold uppercase tracking-widest text-xs">Data unavailable</p>
            <p className="text-muted-foreground/60 text-sm mt-2">Couldn't load this section.</p>
          </div>
        )}
      </div>
    </div>
