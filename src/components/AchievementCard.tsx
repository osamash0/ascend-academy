    ...
    >
      ...
      <div className="flex flex-col gap-6 relative z-10">
        <div className="flex items-start justify-between">
          <div
            className={`w-16 h-16 rounded-[24px] flex items-center justify-center text-3xl flex-shrink-0 transition-transform duration-500 group-hover:scale-110 overflow-hidden ${
              isLocked 
          ...
          >
            {isLocked ? (
              ...
              <img src={displayIcon} className="w-full h-full object-cover" alt={name} />
            ) : (
          ...
          {!isLocked && (
            <div className="flex flex-col items-end">
              <div className="flex items-center gap-1.5 bg-xp/10 px-2 py-1 rounded-lg border border-xp/20">
                ...
                <span className="text-[10px] font-bold text-xp uppercase tracking-tighter">{t('achievementCard.earned')}</span>
              </div>
            </div>
          )}
        </div>
        ...
        <div className="space-y-2">
          <h3 className={`font-bold text-lg tracking-tight ${
            ...
            {isLocked ? t('achievementCard.lockedTitle') : name}
          </h3>
          <p className="text-sm text-muted-foreground line-clamp-2 leading-relaxed opacity-70 group-hover:opacity-100 transition-opacity">
            {isLocked ? t('achievementCard.lockedDescription') : description}
          </p>
        </div>
        ...
        {formattedDate && (
          <div className="pt-4 border-t border-white/5 flex items-center justify-between">
            <div className="flex items-center gap-2 text-[10px] font-bold uppercase tracking-[0.2em] text-primary/60">
              ...
              <span>{t('achievementCard.earnedOn', { date: formattedDate })}</span>
            </div>
        ...
        {isLocked && (
          <div className="pt-4 border-t border-white/5">
             <div className="flex items-center gap-2 text-[10px] font-bold uppercase tracking-[0.2em] text-muted-foreground/30">
              ...
              <span>{t('achievementCard.locked')}</span>
            </div>
          </div>
        )}
      </div>
