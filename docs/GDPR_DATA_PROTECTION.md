...
| **Cerebras** (`CEREBRAS_API_KEY`) | Slide text/content | US | SCCs (Cerebras standard DPA) | Default `PARSER_LLM_MODEL=cerebras` — the primary bulk-parse path for every uploaded lecture. Called **directly** over HTTPS from `backend/services/ai/orchestrator.py`; the LiteLLM gateway this row previously named was removed 2026-08-25 and never carried live traffic. |
