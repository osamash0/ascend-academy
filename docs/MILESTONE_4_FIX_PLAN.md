...
| M54 — 44 legacy un-versioned API call sites vs 23 on `/api/v1`, held together by a 307 redirect shim | The audit itself explicitly warns: "do NOT bundle this with a quick win (~3h, medium risk)." It's not a correctness bug today (the redirect shim works), just an efficiency/cleanliness debt item touching ~67 call sites across the frontend. Given the hour and the explicit risk callout, I'm leaving this as its own dedicated future session rather than rushing a mechanical migration across that many call sites overnight. |
| R40 — `VITE_FEATURE_GLOBAL_SEARCH`/`FEATURE_GLOBAL_SEARCH` undefined everywhere, permanently disabling a fully-built search feature (⌘K palette, in-course search) | Same underlying decision as M56 (already in this table, folded into the A-S3+D deferred bucket above): ship it by setting both env vars, or delete the feature. This is a product call about a whole feature's fate, not a bug fix — I won't unilaterally turn on a previously-off feature flag in prod-adjacent config while you're not here to see the result. |
...
| 26 | Course title/progress/link consistency | M38, M40, M43 | Course title translated on the card list only (not the other 4 surfaces); course progress rollup stuck at 0% while lecture cards showed real progress; library cards not linked to `/course/:id` | ✅ | [#47](https://github.com/osamash0/ascend-academy/pull/47) — progress is now a real weighted average of per-lecture completion, not gated on any single lecture hitting 100%; added an explicit "view course" affordance separate from the tile's existing click/double-click handlers |
| 27 | Responsive sweep | M23, M25, M52, M53 | Exam answer clipped ≤1054px; mobile dashboard nudge overlaps h1; professor editor doesn't collapse on mobile; `/course-v3` blank vertical space | ✅ | [#45](https://github.com/osamash0/ascend-academy/pull/45) — all 4 verified live at the cited breakpoints |
| 28 | Branding/auth polish + flaky test | M63, M72, M73 | 3 different logo treatments across Landing/Auth/Impressum; password placeholder dots look pre-filled; flaky Onboarding test | ✅ | [#44](https://github.com/osamash0/ascend-academy/pull/44) — standardized on Landing's cyan/blue Layers mark; also found and fixed the same broken pattern on `Datenschutz.tsx` (a dead `gradient-primary` CSS class rendering no background at all) while checking for other pages; M63 stress-tested 30+ times under artificial CPU load after the fix |
| 29 | Exam feature reliability | R9, R30, R31, R32, R33 | Exam fetch errors render a blank page; timer counts up with no countdown/auto-submit; raw API error strings shown to students; answer-save race can revert the latest selection; review due-count uncapped vs the 100-card session cap | ✅ | [#46](https://github.com/osamash0/ascend-academy/pull/46) — highest regression risk in this batch; personally reviewed the full diff line-by-line before merging (confirmed `ApiError` stays backward-compatible with existing `.message` call sites, the client's `GRACE_SECONDS`/auto-submit logic matches the backend's actual late-submission grading behavior, R33 reuses the existing `DEFAULT_TOTAL_CAP`); agent also live-verified all 5 fixes against a real seeded test account, including reproducing the original R32 race live to confirm the fix |
...
## Sessions 32+ (audit gap sweep)

After the session-21+ batch, ran a full gap analysis of every M1–M81/R1–R54 finding against this doc to catch anything missed. Found a few already resolved without being logged here, and dispatched fixes for the rest.

**Already fixed, no action needed (found during the gap sweep):**
- **M6** — `/profiles` refetch storm (60 req/s idle) — fixed pre-audit, commit `24dc9a2`, confirmed still in `main`'s history.
- **M10** — onboarding dead-end — fixed pre-audit, commit `f682bde`.
- **M11** — upload wizard silent CTA failure — fixed pre-audit, commit `cd426cf`.
- **M35** — batch slide-synthesis envelope bug (100% failure rate on the batched path) — fixed 2026-08-17, before this audit ran. Residual: no metric/alert on the fallback path — low priority, not actioned tonight.
- **R51** (`total_slides` desyncs from real slide count) and **R52** (stalled `extracting` jobs had already finished the work but hung before finalization — "reframes M16") — both already fixed by tonight's session #14 (R+X, PR #37): `backend/services/parser/persist.py`'s `sync_total_slides()` recomputes the column from a live `COUNT(*)` and is called by the same reconciliation path that handles stuck jobs. The code's own docstring cites R51/R52 by ID. No further action needed.
- **M18** (student accounts render the full professor toolset, no client-side gate) — investigated: the audit's own repo-side check (A §6) found this is **deliberate** — `allowedRoles: [professor, student]` on those specific routes is intentional support for student-authored course creation, while genuinely professor-only routes (dashboard, archive, analytics) are correctly gated. The audit itself downgrades this to "reduces to a UI-consistency finding; M17 is the actual security question" (M17 is already in the "needs your input" table above). No code change made — confirmed working as designed.
